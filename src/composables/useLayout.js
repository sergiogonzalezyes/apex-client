...
const appState = ref({
	primary: localStorage.getItem('primaryApex') || 'emerald',
	surface: localStorage.getItem('surfaceApex') || 'slate',
	darkMode: localStorage.getItem('isDarkModeApex') === 'true'
});
...
export function useLayout() {
	...
	watch(() => appState.value.darkMode, (val) => {
		localStorage.setItem('isDarkModeApex', val);
	});

	watch(() => appState.value.primary, (val) => {
		localStorage.setItem('primaryApex', val);
	});

	watch(() => appState.value.surface, (val) => {
		localStorage.setItem('surfaceApex', val);
	});
	
	const savedPrimary = primaryColors.value.find(p => p.name === appState.value.primary);
	if (savedPrimary) updatePrimaryPalette(savedPrimary.palette);

	const savedSurface = surfaces.value.find(s => s.name === appState.value.surface);
	if (savedSurface) updateSurfacePalette(savedSurface.palette);
